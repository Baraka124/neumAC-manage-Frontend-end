# neumDesk V46.14 — Staff Phase 4 UI Foundation Candidate
...
Phase 4 is a **candidate checkpoint** until authenticated browser validation confirms the four representative Person profiles still behave correctly: Attending, Internal Resident, Rotating Resident and External Resident.
...
8. **No runtime rewrite.** `app.js`, `grounded-core.js` and `activity45.js` remain byte-for-byte unchanged in this candidate.
...
The complete historical suite has been rerun on this candidate: **24 suites / 387 checks passed**, including the browser-DOM template-safety guard and 18 dedicated Phase 4 checks. Live authenticated four-profile comparison remains the promotion gate.
