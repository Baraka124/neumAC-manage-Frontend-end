...
(function (root, factory) {
  ...
  async function load(request,allowed,signal,scope={}) {
    const results=await Promise.all(specs.map(async s=>{
      ...
      try { return [s.key,{state:'ready',rows:(await fetchRows(request,path,signal)).filter(r=>!r.deleted_at),label:s.label,checkedAt:new Date().toISOString()}]; }
      catch (_) { return [s.key,{state:'unavailable',rows:[],label:s.label}]; }
    }));
  ...
  function build(person,start,end,sources,selected={oncall:true,rotations:true,studies:true,projects:true,lines:true}) {
    ...
    function event(r,kind,title,role,s,e,detail) {
      ...
      model.events.push({kind,title,role,start:from,end:to,detail:detail||'',ref:String(r.id),source:kind==='oncall'?'On-call duties':kind==='rotation'?'Rotations':kind==='study'?'Research studies':'Innovation projects'});
    }
    ...
    function portfolio(key,leadField,leadRole) {
      ...
      rows(key).forEach(r=>{
        ...
        const item={title:r.title||'Untitled record',role:[...new Set(roles)].join(' · '),status:nice(r.status||r.current_stage||r.development_stage),line:lineName(r.research_line_id),start:s,end:e,ref:String(r.id),timing:missing?'Period relevance unverified — dates incomplete or invalid':'Recorded dates overlap the selected period'};
        model[key].push(item);
  ...
  function render(m,demo=false) {
    ...
    ${portfolio('studies','Research involvement','03')}${portfolio('projects','Innovation involvement','04')}
