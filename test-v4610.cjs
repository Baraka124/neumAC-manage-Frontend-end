...
const tests=[
 ['V46.10 markers and core version',()=>{assert(/neumDesk · V46\.(?:10|11)/.test(html));assert(/46\.(?:10-leave-action-integrity|11-rotation-action-integrity)/.test(html));assert(/^46\.(?:10|11)$/.test(Core.VERSION))}],
 ...
 ['DepartmentOS living ledger records V46.10',()=>{assert(/Implementation checkpoint: V46\.(?:10|11)/.test(arch));assert(arch.includes('Leave Action Integrity'));assert(arch.includes('Resident Rotation Action Integrity'))}],
