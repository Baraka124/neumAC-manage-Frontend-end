...
const tests=[
 ['V46.10 markers and core version',()=>{assert(html.includes('neumDesk · V46.10'));assert(html.includes('46.10-leave-action-integrity'));assert.strictEqual(Core.VERSION,'46.10')}],
 ...
 ['Grounded architecture records Leave as third adapter',()=>{assert(garch.includes('Leave is the third'));assert(garch.includes('leave.commit_absence'));assert(garch.includes('Resident Rotation Action Integrity'))}],
 ['DepartmentOS living ledger records V46.10',()=>{assert(arch.includes('Implementation checkpoint: V46.10'));assert(arch.includes('Leave Action Integrity'));assert(arch.includes('Resident Rotation Action Integrity'))}],
