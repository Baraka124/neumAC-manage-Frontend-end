...
const tests = [
  ...
  ['dashboard-aligned command header exists', () => { assert(html.includes('cu465-command-hero') || html.includes('dbh-frame cu469-hero')); assert(css.includes('V46.5 · Clinical Units — Operational Intelligence') || css.includes('V46.9 · CLINICAL UNITS DOMAIN SEMANTICS + DASHBOARD HEADER ALIGNMENT')); assert(html.includes('clinicalUnitHeaderMetrics')); assert(html.includes('clinicalUnitAttentionItems')) }],
