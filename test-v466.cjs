...
const tests=[
 ['V46.6 build and cache markers',()=>{assert(html.includes('neumDesk · V46.6')||html.includes('neumDesk · V46.7')||html.includes('neumDesk · V46.8')||html.includes('neumDesk · V46.9')||html.includes('neumDesk · V46.10') || html.includes('neumDesk · V46.11') || html.includes('neumDesk · V46.12') || html.includes('neumDesk · V46.13'));assert(html.includes('app.js?v=46.6-clinical-units-detail')||html.includes('app.js?v=46.7-clinical-units-adaptive')||html.includes('app.js?v=46.8-grounded-architecture')||html.includes('app.js?v=46.9-grounded-action-integrity')||html.includes('app.js?v=46.10-leave-action-integrity') || html.includes('app.js?v=46.11-rotation-action-integrity') || html.includes('app.js?v=46.12-clinical-units') || html.includes('app.js?v=46.13-portfolio-intelligence'));assert(html.includes('style.css?v=46.6-clinical-units-detail')||html.includes('style.css?v=46.7-clinical-units-adaptive')||html.includes('style.css?v=46.8-grounded-architecture')||html.includes('style.css?v=46.9-grounded-action-integrity')||html.includes('style.css?v=46.10-leave-action-integrity') || html.includes('style.css?v=46.11-rotation-action-integrity') || html.includes('style.css?v=46.12-clinical-units'))}],
 ['application JavaScript parses',()=>{new vm.Script(app)}],
 ['canonical unit operational snapshot is exposed',()=>{assert(app.includes('const unitDetailSnapshot = computed'));assert(app.includes('unitDetailSnapshot, unitDetailCapacityMonths'));assert(html.includes('Resident capacity today'));assert(html.includes('Attending physicians'))}],
 ...
 ['unit team error remains distinct from empty membership',()=>{assert(html.includes('links unavailable') || html.includes('Attending links'));assert(html.includes('No attending physicians linked to this unit'));assert(html.includes("loadUnitStaff(unitDetailDrawer.unit.id,{force:true})"))}],
 ...
 ['placement readiness distinguishes operational readiness',()=>{assert(app.includes('const operationalReady = fullFit'));assert(html.includes('attending') || html.includes('Attending'));assert(css.includes('.cu465-placement-readiness'))}],
 ...
 ['unit drawer surfaces attention states',()=>{assert(app.includes("title:'Resident capacity exceeded'"));assert(html.includes('unitDetailSnapshot?.alerts?.length'))}],
