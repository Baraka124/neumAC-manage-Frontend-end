...
test('P3-01 phase marker and cache keys advance',()=>{assert(html.includes('STAFF PROFILE DRAWER · V46.14 PHASE 3'));assert(html.includes('app.js?v=46.14-staff-phase3-adaptive-residents'));assert(html.includes('style.css?v=46.14-staff-phase3-adaptive-residents'))});
...
test('P3-05 origin to host route is rendered',()=>{assert(html.includes('pp3-route'));assert(html.includes('Host / current rotation'));assert(html.includes('originLabel'))});
...
test('P3-16 architecture advances to Staff Phase 3',()=>{assert(arch.includes('Architecture v2.5'));assert(arch.includes('Staff Phase 3 — Adaptive resident profile'))});
...
let passed=0;for(const [n,f] of tests){try{f();console.log('PASS',n);passed++}catch(e){console.error('FAIL',n,e.message);process.exit(1)}}console.log(`${passed} V46.14 Staff Phase 3 checks passed.`)
