...
const tests=[
 ['V46.9 markers and core version',()=>{assert(/neumDesk · V46\.(?:9|10|11|12)/.test(html));assert(/46\.(?:9-grounded-action-integrity|10-leave-action-integrity|11-rotation-action-integrity|12-clinical-units)/.test(html));assert(/^46\.(?:9|10|11|12)$/.test(Core.VERSION))}],
 ...
 ['DepartmentOS living architecture records V46.9 checkpoint',()=>{assert(/Implementation checkpoint: V46\.(?:9|10|11|12)/.test(arch));assert(arch.includes('Grounded Action Integrity'));assert(/Resident Rotation|What comes next|Next migration/.test(arch))}],
